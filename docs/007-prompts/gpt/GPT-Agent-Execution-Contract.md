...
| Ruta                    | `docs/007-prompts/gpt/GPT-Agent-Execution-Contract.md`                                                                                                                                                                                                                                |
| Estado                  | Draft Implementable                                                                                                                                                                                                                                                                     |
| Versión                | 0.1.0                                                                                                                                                                                                                                                                                   |
| Owner                   | ORION Architecture                                                                                                                                                                                                                                                                      |
| Última actualización  | 2026-07-02                                                                                                                                                                                                                                                                              |
| Documentos relacionados | `docs/004-agentes/`, `docs/007-prompts/gpt/README.md`, `docs/007-prompts/gpt/00-gpt-global-system.md`, `docs/007-prompts/000-shared/agent-base-contract.md`, `docs/007-prompts/000-shared/agent-output-standards.md`, `docs/007-prompts/000-shared/editorial-guardrails.md` |

---

## 1. Propósito

Este documento define el contrato estándar para ejecutar agentes XMIP usando **GPT** como runtime cognitivo.
...
GPT no debe ejecutar agentes como conversación libre.

GPT debe ejecutar agentes bajo contrato.
...
Este contrato aplica a todos los adaptadores GPT ubicados en:
...
docs/007-prompts/gpt/ = adaptación del agente a GPT
docs/007-prompts/000-shared/ = contratos y guardrails compartidos
```

GPT debe ejecutar la tarea respetando esta separación.

---

## 4. Identidad de ejecución

Toda ejecución GPT debe declarar:
...
Si no existe `execution_id`, GPT puede proponer uno en formato:

```text
gpt-<agent-short-code>-YYYYMMDD-001
```

Ejemplos:

```text
gpt-hns-20260702-001
gpt-hsv-20260702-001
gpt-hed-20260702-001
gpt-hmi-20260702-001
```
...
Antes de ejecutar cualquier agente, GPT debe considerar como fuentes de contrato:
...
Si falta un campo obligatorio, GPT debe responder con:
...
GPT puede ejecutar acciones cognitivas como:
...
GPT no debe ejecutar acciones externas.
...
```text
GPT prepara outputs.
GPT no ejecuta acciones externas finales.
```
...
GPT no debe producir recomendaciones financieras ni señales de trading.
...
GPT debe bloquear cualquier salida que exponga secretos.
...
GPT debe seguir este protocolo:

```text
1. Identificar agente
2. Identificar runtime como GPT
3. Validar input mínimo
4. Cargar contrato aplicable
5. Aplicar definición oficial del agente
6. Aplicar guardrails compartidos
7. Separar hechos, claims, inferencias e incertidumbre
8. Ejecutar tarea cognitiva
9. Generar output estructurado
10. Marcar riesgos
11. Definir handoff
12. Marcar human_review_required
13. Declarar limitaciones
```
...
Cuando GPT no pueda completar una tarea plenamente, debe declarar:
...
Antes de finalizar, GPT debe validar:
...
Una ejecución GPT es exitosa cuando:
...
```text
Eres GPT ejecutando un agente XMIP bajo contrato.
...
- GPT Agent Execution Contract
- reglas compartidas
- adaptador GPT específico del agente
...
| -------- | ---------: | ----------------------------------------------------------- | ------------------ |
| 0.1.0    | 2026-07-02 | Creación inicial del contrato de ejecución de agentes GPT | ORION Architecture |
